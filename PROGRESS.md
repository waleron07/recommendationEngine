...
> Состояние: **Этапы 0–6 закрыты. Следующий — Этап 6а (`@recoengine/testing`).**
> Последняя ревизия: закрыт **Этап 6** — три модификатора §15 в `@recoengine/modifiers` (16 тестов, кривые затухания/восстановления), ARCHITECTURE.md §15.1 сведён с кодом тем же заходом.
...
Ожидаемо: **597 тестов в 32 файлах**, `✓ Architecture check passed (7 packages)`, покрытие ядра **95.4% строк / 93.26% ветвей** (пороги в `vitest.config.ts`: 90/85/90/90). Из них 21 — golden-тесты Этапа 5 (`packages/strategies/src/strategies.test.ts`), 16 — Этапа 6 (`packages/modifiers/src/modifiers.test.ts`).
...
Три пакета (`features`, `diversity`, `testing`) — пустые каркасы с `export {}`. `strategies` (Этап 5) и `modifiers` (Этап 6) закрыты. Шестой, `recoengine`, — фасад `export * from '@recoengine/core'`; стратегии и модификаторы он пока не реэкспортирует (решить при Этапе 11, когда появится, что публиковать).
...
## 4. Следующий шаг — Этап 6а (`@recoengine/testing`)

Критерий приёмки из §22: **порт проверяется на контракт одной строкой**; обязательно входят реакция на прерванный `ctx.signal` (§17.1) и поведение под обеими `errorPolicy` (§17.2). Это contract-test kit из §20–§21 плюс общие фикстуры и golden-раннер.

Что учесть, заходя:

1. **Долг, который kit гасит прямо.** Сейчас каждый тест (golden-наборы Этапов 5 и 6) катает синтетические экстрактор/провайдер/движок руками — `itemFeatures`, `provider`, `listen`, фиксированный `CLOCK`. Это уже трижды продублировано; вынести в `@recoengine/testing` как переиспользуемые фикстуры — и переписать существующие тесты на них тем же заходом, иначе kit заведут, а дублирование останется.
2. **Contract-тест — это набор, параметризуемый портом.** `contractOf(port)` прогоняет фиксированные проверки: прерванный сигнал → `AbortError` за отведённое время (§17.1, правило 3); брошенное исключение → корректная реакция под `strict` и `degrade` (§17.2). Сторонний плагин подключает набор одной строкой — это и есть обещание §20.
3. **Property-хелперы (§21).** `fast-check` уже devDependency: нормализатор ∈ [0..1] и не `NaN`; детерминизм при фиксированном seed; веса не меняют масштаб. Часть уже покрыта в ядре — kit выносит общие генераторы, чтобы плагины проверяли то же на себе.
4. **Правило зависимостей.** `testing` — publishable-пакет (`core ← testing`), значит не смеет тянуть `examples/*` и не смеет заглядывать внутрь ядра мимо портов. `check-arch` это проверит; следить, чтобы фикстуры строились через публичный API.

**Хвосты Этапов 5–6, оставленные сознательно:** `softmax` по-прежнему не написан (§5, никому пока не понадобился); `SimilarityProvider` без слота в `Registry` — решать на Этапе 7 (diversity), там же MMR; `recoengine`-фасад стратегии и модификаторы не реэкспортирует. `recovery()` из `decay.ts` модификатор не использует (восстановление выражено затуханием счётчика) — остаётся для линейной формы.
