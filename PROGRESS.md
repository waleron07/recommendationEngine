...
> Состояние: **Этапы 0–8 закрыты. Следующий — Этап 8а (`@recoengine/features`).**
> Последняя ревизия: закрыт **Этап 8** — explainability в ядре: округление презентационной шкалы, `ScoreTrace` при `full`, `engine.explain(itemId)` (12 тестов + контракт `assertExplanationSums`). Закрыты три долга §5. ARCHITECTURE.md §16.1 сведён тем же заходом.
...
Ожидаемо: **646 тестов в 35 файлах**, `✓ Architecture check passed (7 packages)`, покрытие ядра **95.4% строк / 93.26% ветвей** (пороги в `vitest.config.ts`: 90/85/90/90). Из них 30 — Этап 5 (`strategies.test.ts`), 19 — Этап 6 (`modifiers.test.ts`), 11 — Этап 6а (`testing.test.ts`), 17 — Этап 7 (`diversity.test.ts`), 12 — Этап 8 (`engine/explainability.test.ts`).
...
## 4. Следующий шаг — Этап 8а (`@recoengine/features`)

Критерий приёмки из §22: **доменно-нейтральные фичи переиспользуются music и ecommerce**. Последний пустой каркас (`export {}`); общие экстракторы/трансформы, которые не знают домена, но считают то, что нужно стандартным стратегиям.

Что учесть, заходя:

1. **Что вообще доменно-нейтрально.** Экстрактор из §11.3 читает `Item.payload` (домен), но часть входов домену безразлична: `item_age` из `Item`-timestamp и `ctx.now`; `interaction_count`/`interaction_recency` из `ctx.history` (порт `HistoryIndex`, не payload); `popularity_*` — если хост кладёт счётчик в известное поле. Вынести те, что считаются из истории/времени, а не из payload, — они одинаковы для музыки и магазина.
2. **`FeatureTransform` для производных.** §8 включает engineering-стадию (4a): нормализация возраста, лог-счётчиков, комбинации. Трансформ читает уже извлечённые колонки — доменно-нейтрален по построению.
3. **Прогнать через contract-kit.** Экстрактор — порт; `assertExtractorErrorPolicy` (§17.2) и отмена проверяются одной строкой (Этап 6а).
4. **Правило зависимостей.** `features` — publishable (`core ← features`), не тянет `examples/*`, строится через публичный API. `check-arch` проверит.
5. **`recoengine`-фасад.** После features стоит решить, реэкспортит ли «с батарейками»-пакет strategies/modifiers/diversity/features (§18) — или это Этап 11. Пока не реэкспортирует.

**Хвосты, оставленные сознательно:** `softmax` не написан (§5, никому пока не понадобился); `recovery()` из `decay.ts` не используется (восстановление выражено затуханием счётчика) — остаётся для линейной формы. Остаточные долги §5 (`container.child()`, приоритет §10, product-комбайнер, underflow `cosine`) — не блокируют, разобраны в §5. Доменные примеры (Этапы 9–10) — после features.
