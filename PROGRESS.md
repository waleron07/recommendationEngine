...
**CI зелёный, матрица подтверждена на практике:** Node 20/22/24, Bun, Deno плюс отдельная джоба lint/arch/build/test, на каждый push в `main`. Оговорка: джобы рантаймов гоняют `scripts/smoke.mjs` (34 строки), а не весь набор — то есть матрица доказывает «пакет грузится и работает везде», но не «672 теста зелены везде».
...
benchmarks/math.bench.ts    вне verify: меряет на 1k/10k/100k кандидатов
```

Все пять plugin-пакетов построены: `strategies` (Этап 5), `modifiers` (Этап 6), `testing` (Этап 6а), `diversity` (Этап 7), `features` (Этап 8а). Пустых каркасов больше нет. Шестой, `recoengine`, — фасад «с батарейками»: реэкспортит core + все плагины (Этап 11), так что `npm i recoengine` даёт всё. Два доменных примера: `examples/music` (Этап 9) и `examples/ecommerce` (Этап 10). Примеры ничего в `packages/` не добавляют. Оба доказали приёмочный критерий: `git status packages/core/` пуст. Все пакеты на версии `0.1.0` (Этап 11), публикация ждёт auth.
...
### Бенчмарки (на 1k / 10k / 100k кандидатов — §22; расширены на Этапе 11)

Размеро-зависимые бенчи гоняются на трёх точках, чтобы поймать деградацию сложности: линейный проход выглядит нормально на 1k и падает на 100k. Разрыв растёт **с размером** — это и есть сигнал регресса, а не любое отдельное число.

| Что | 1k | 10k | 100k |
|---|---|---|---|
| `topK` (n log k) против `full sort` (n log n), → страница 20 | **31× быстрее** | **74×** | **109×** |
| `minmax` (один проход) против `rank`/`rrf` (сортируют) | ~2k/с vs `rank` | расходятся | `rank` в ~50× дороже |
| `cosine`/`dot`/`jaccard` по 128 измерениям (размеронезависимы) | — меряются один раз, ~млн/с | | |

Растущий разрыв topK↔sort (31→74→109×) подтверждает, что куча оправдывает свою сложность; сужение было бы сигналом, что оптимизация сломалась. `pnpm bench` — вне `verify` (меряет, не утверждает).
...
**Опциональное к релизу — сделано (Этап 11):** typedoc генерит все 7 пакетов (добавлен `recoengine`); бенчмарки расширены на 1k/10k/100k (§22); браузерный прогон заведён в CI (job `browser`, headless Chromium, §23.5). Осталось необязательное: `@document`-описания в typedoc, автопубликация docs на GitHub Pages.

**Хвосты, оставленные сознательно:** `recovery()` из `decay.ts` не используется. Остаточные долги §5 после чистки Этапа 11: `container.child()` (request-скоуп не вызывается), product-комбайнер (своя свёртка теряется при переоткрытии доски), underflow `cosine` (<1.5e-162, недостижимо), `FeatureCache`/`retrievalScore` (объявлены, не подключены) — не блокируют `v0.1.0`, перечислены в CHANGELOG как известные ограничения. Закрыты на Этапе 11: `softmax`, инверсия §10, коллизия id в `graph.ts`, `null` в `merge()`.
...
| ~~`softmax` не написан~~ | — | **Закрыто (Этап 11):** `softmax`/`softmaxScaled` (стабильный, вычитание max; температура), в `NORMALIZERS` и публичном API |
...
| ~~§10: приоритет инвертирован~~ | — | **Закрыто (Этап 11):** override-веса переприменяются поверх `WeightProvider` (`reapplyRequestWeights`) — вызывающий (слой 1) побеждает провайдера (слой 2); регресс-тест |
| ~~`graph.ts`: id экстрактора = id трансформа~~ | — | **Закрыто (Этап 11):** рёбра по `FeatureKey → FeatureTransform` (только трансформы), не по id владельца — коллизия id больше не даёт ложный цикл; регресс-тест |
| ~~`merge()` считает `null` значением~~ | — | **Закрыто (Этап 11):** `null` в патче = «оверрайда нет» (как `undefined`) — пустое YAML-тело сохраняет дефолт, не падает; регресс-тест |
...
**Что осталось открытым после чистки Этапа 11 — и почему не тронуто.** Шесть долгов закрыто (softmax, §10-приоритет, коллизия id в `graph.ts`, `null` в `merge()`, покрытие `ranking.ts`, плюс на Этапах 7–8 — блендер, SimilarityProvider, округление, ScoreTrace, explain). Оставшиеся — **не «не успели», а осознанно отложены**: `container.child()` (request-скоуп) и `FeatureCache`/ключ-кэша/`retrievalScore` — инфраструктура под фичи, которых ещё нет; заводить её без потребителя спекулятивно, это post-1.0. Product-комбайнер и underflow `cosine` — **won't-fix с обоснованием**: первый выразим через weighted-sum/RRF, второй потребовал бы третий проход по самой горячей функции (MMR зовёт её O(page×pool) раз) ради случая, недостижимого на реальных данных. Все перечислены в CHANGELOG `v0.1.0` как известные ограничения.

### Инфраструктура

| Что | Состояние |
|---|---|
| **CI** | ✅ Зелёный: Node 20/22/24, Bun, Deno + **браузер** (headless Chromium). Оговорка — рантаймы гоняют smoke, а не весь набор |
| **Remote** | ✅ `waleron07/recommendationEngine`, публичный |
| **Браузерный прогон** | ✅ Заведён (Этап 11): job `browser` грузит собранный ESM ядра в headless Chromium через playwright (ставится эфемерно в job, не в манифест) — `scripts/smoke-browser.mjs` |
| **Тесты не типизируются** | `tsconfig` исключает `*.test.ts`; ошибка типов в тесте не упадёт в `verify` |
| **ARCHITECTURE.md** | Сверена (§22 отражает Этапы 0–11) |
| **Biome** | ✅ 0 предупреждений (с Этапа 11) |
