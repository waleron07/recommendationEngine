...
> Состояние: **Этапы 0–9 закрыты (все пакеты + первый доменный пример). Следующий — Этап 10 (`examples/ecommerce`).**
> Последняя ревизия: закрыт **Этап 9** — `examples/music` (музыкальный домен: 5 экстракторов + датасет + `buildMusicEngine`; 7 e2e-тестов). **Критерий доказан буквально: 0 изменений в `core`.** ARCHITECTURE.md §22 (домен схлопнут в пример) сведён тем же заходом.
...
Ожидаемо: **663 теста в 37 файлах**, `✓ Architecture check passed (7 packages)`, покрытие ядра **95.4% строк / 93.26% ветвей** (пороги в `vitest.config.ts`: 90/85/90/90). По этапам: 30 — Этап 5, 19 — Этап 6, 11 — Этап 6а, 17 — Этап 7, 12 — Этап 8, 10 — Этап 8а, 7 — Этап 9 (`examples/music/src/music.test.ts`). `vitest` теперь ловит и `examples/*/src/**/*.test.ts` (не только `packages/*`).
...
**CI зелёный, матрица подтверждена на практике:** Node 20/22/24, Bun, Deno плюс отдельная джоба lint/arch/build/test, на каждый push в `main`. Оговорка: джобы рантаймов гоняют `scripts/smoke.mjs` (34 строки), а не весь набор — то есть матрица доказывает «пакет грузится и работает везде», но не «656 тестов зелены везде».
...
Все пять plugin-пакетов построены: `strategies` (Этап 5), `modifiers` (Этап 6), `testing` (Этап 6а), `diversity` (Этап 7), `features` (Этап 8а). Пустых каркасов больше нет. Шестой, `recoengine`, — фасад `export * from '@recoengine/core'`; готовые пакеты он пока не реэкспортирует (решить при Этапе 11, когда появится, что публиковать). Первый доменный пример — `examples/music` (Этап 9); дальше `examples/ecommerce` (Этап 10). Примеры ничего в `packages/` не добавляют.
...
## 4. Следующий шаг — Этап 10 (`examples/ecommerce`)

Критерий приёмки из §22: **0 изменений в core** — приёмочный тест архитектуры. Второй домен (товары) обязан обойтись новыми экстракторами и датасетом, переиспользуя те же стратегии/модификаторы/диверсификаторы, что и музыка. Если для e-commerce потребуется тронуть ядро — абстракция протекла, чинить до релиза.

Что учесть, заходя:

1. **Что переиспользуется как есть.** Стратегии, модификаторы, `@recoengine/diversity`, `@recoengine/features` — те же. Меняются только доменные экстракторы: `payload.brandId → affinity_brand`, `payload.categoryId → affinity_category`, `payload.salesCount → popularity_global`, `payload.addedAt → item_age`. Зеркало музыкального примера — это и есть доказательство: «поменяй экстракторы, остальное держится».
2. **Домен-специфика e-commerce.** Событие «покупка» весомее «просмотра» — можно параметризовать `interactionCountExtractor({ eventType: 'purchase' })` или взвесить. `AttributeQuotaDiversifier` по бренду/категории («не 20 товаров одного бренда»). Тот же паттерн, другие поля.
3. **Что доказать тестом.** Сквозной путь + `engine.explain()` на товаре; главное — `git status packages/core/` пуст. Можно вынести общее с music (buildEngine-паттерн) — но не в `core`, а в сам пример; дублирование двух маленьких датасетов допустимо, это и есть демонстрация.
4. **Грабли из Этапа 9 — уже известны.** `examples/*` в `vitest`/`tsconfig` (сделано, распространяется на новый пример); `env.d.ts` для `console`/`process`; devDep-симлинки + lockfile офлайн сразу.

**Хвосты, оставленные сознательно:** `softmax` не написан (§5, никому пока не понадобился); `recovery()` из `decay.ts` не используется; `recoengine`-фасад готовые пакеты не реэкспортирует (решить на Этапе 11). После Этапа 10 — Этап 11: docs, typedoc, бенчмарки, `v0.1.0`, публикация в npm. Остаточные долги §5 (`container.child()`, приоритет §10, product-комбайнер, underflow `cosine`) — не блокируют, разобраны в §5.
