...
> Состояние: **Этапы 0–10 закрыты (все пакеты + оба доменных примера). Следующий — Этап 11 (docs, бенчмарки, `v0.1.0`, публикация).**
> Последняя ревизия: закрыт **Этап 10** — `examples/ecommerce` (второй домен: 5 экстракторов с взвешиванием типов событий + prefilter «уже куплено»; 7 e2e-тестов). **Приёмочный тест пройден: 0 изменений в `core`** — те же стратегии/модификаторы/диверсификаторы, что и music. ARCHITECTURE.md §22 сведён.
...
Ожидаемо: **670 тестов в 38 файлах**, `✓ Architecture check passed (7 packages)`, покрытие ядра **95.4% строк / 93.26% ветвей** (пороги в `vitest.config.ts`: 90/85/90/90). По этапам: 30 — Этап 5, 19 — Этап 6, 11 — Этап 6а, 17 — Этап 7, 12 — Этап 8, 10 — Этап 8а, 7 — Этап 9 (`examples/music`), 7 — Этап 10 (`examples/ecommerce`). `vitest` ловит и `examples/*/src/**/*.test.ts`.
...
## 4. Следующий шаг — Этап 11 (docs, бенчмарки, `v0.1.0`, публикация)

Критерий приёмки из §22: **публикация в npm**. Последний этап. Всё построено и проверено; остаётся довести до релиза.

Что учесть, заходя:

1. **Фасад `recoengine`.** Сейчас `export * from '@recoengine/core'` — готовые пакеты (strategies/modifiers/diversity/features) НЕ реэкспортит. §18 обещает «с батарейками». Решить: реэкспортить их из `recoengine`, чтобы `npm i recoengine` давал всё. Тогда `recoengine` зависит от них (уже в `ALLOWED_DEPS` `check-arch`).
2. **Публикация из CI (§5, npm-раздел).** Обычный токен НЕ сработает при 2FA `auth-and-writes` — нужен granular access token либо Trusted Publishing через OIDC. Организация `recoengine` создана, `waleron` — owner. Версии проставить через changesets (уже в devDeps).
3. **typedoc.** `pnpm docs` настроен (`typedoc.json`); проверить, что генерится по всем пакетам, а не только core.
4. **Бенчмарки.** `benchmarks/math.bench.ts` есть (§22 просит 1k/10k/100k + регрессия перфа в CI). Расширить набор или задокументировать текущие числа в README (уже есть таблица).
5. **README `Install`.** После публикации секция становится `npm i recoengine` (сейчас «build from source»). Обновить.
6. **Оставшиеся 4 предупреждения биома** (§1) — почистить перед релизом: неиспользуемый `P` в `RecommendationRequest`, `policy` в `normalize()`.

**Хвосты, оставленные сознательно:** `softmax` не написан (§5, никому пока не понадобился); `recovery()` из `decay.ts` не используется. Остаточные долги §5 (`container.child()`, приоритет §10, product-комбайнер, underflow `cosine`) — не блокируют релиз `v0.1.0`, но стоит перечислить в CHANGELOG как известные ограничения. Браузерный прогон (§23.5) в CI не заведён — решить, нужен ли для 0.1.0.
