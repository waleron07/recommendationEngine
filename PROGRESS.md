...
> Последняя ревизия: аудит Этапов 2–4 — найдено и исправлено **10 багов**, см. §5.
...
Код осознанно разошёлся с документом в **31 месте**. Документ не переписан — при следующей ревизии внести в §3, §5, §6, §8, §10, §12, §17, §19.

### Этап 1

**3.1 Матрица использует два layout'а** (`matrix.ts`). §5.5 требует column-major «везде». Не работает: нормализатор идёт по фиче через все строки (column-major), косинус — по 128 измерениям одной строки (row-major). Реализовано: скаляры column-major, эмбеддинги row-major поблочно.

**3.2 Версия схемы не зависит от порядка регистрации** (`feature.ts`). Хеш от **отсортированного** отпечатка: иначе перестановка в `use()` обнуляла бы кэш фичей.

**3.3 `contributionOf` не делит на Σweights** (`score.ts`). Делит `ScoreBoardBuilder.build()` — только доска знает, какие стратегии отработали. Отсюда бесплатный cold start.
...
**3.5 `platform.d.ts`: AbortSignal объявлен вручную.** `lib: ["ES2022"]` + `types: []` — в ядре нет ни DOM, ни Node-типов, на этом держится машинная изоморфность. Затащить DOM ради одного типа значило бы отдать ядру `fetch` и снять сторожа. **Подтверждено CI:** `AbortSignal.any`/`.timeout` работают на Node 20/22/24, Bun, Deno.

**3.6 Две схемы фичей.** `schema` (кандидаты) + `profileSchema` (пользователь). `affinity_genre` — законное имя для обеих; один namespace назвал бы пару коллизией. Это разные векторные пространства.

**3.7 `ScoreModifier.apply` получает `MutableScoreBoard`.** §6 типизировал `board: ScoreBoard` с возвратом `void` — так модификатор не может сделать ничего.

**3.8 Конфиг валидируется ПОСЛЕ `register()`.** §8.3 ставит валидацию шагом 2. Нельзя: `weights` проверяются против зарегистрированных стратегий.

**3.9 `resolve()` одноразовый при любом исходе.** Повторный прогон поверх полуразрушенного состояния сообщил бы о **повторе**, а не о причине.

**3.10 Прямой `add*/set*` ставится в ту же очередь, что и `use()`** — иначе два пути записи с обратным порядком.

**3.11 Дедуп только для именованных плагинов.** Анонимные отслеживаются в `WeakSet`: две `AffinityStrategy` с одним id молча слились бы.

**3.12 В `EngineConfig` добавлены `errorPolicy` и `filterErrorBudget`** (описаны в §17.2, в интерфейс §10 не внесены).

**3.13 `EngineBuilder.provide(token, value)`** — в §8.0 нет. Без него хост не подставит свой `Clock`.

**3.14 `setWeightProvider`** добавлен в `Registry`: §10 требует порт, слота не было.

**3.15 `UserFeatureExtractor.scope: 'user'`** — литеральный маркер: порт структурно совпадает с `FeatureExtractor`, а `extract.length` ломает параметр по умолчанию.

### Этап 3

**3.16 `DiagnosticWarning` живёт в `domain/recommendation.ts`.** Он — то, что получает вызывающий; рядом с пишущим портом он направил бы зависимость назад.

**3.17 Провайдер уважает `criticality`** — расхождение со *строкой* матрицы §17.2 («degrade: продолжаем на оставшихся»). Иначе поле — декорация. Чтобы получить поведение матрицы, провайдер помечается `optional`.

**3.18 Бюджет retrieval делится между провайдерами.** Три провайдера по 5000 — это потолок 15000.

**3.19 Модификаторы пишут в буфер, коммит — только если долетели.** Иначе половина выдачи приглушена, половина нет — молча.

**3.20 Нормализация не деградируема и проверяется.** Сырая колонка вместо нормализованной положила бы 4 200 000 в сумму чисел из [0..1].

**3.21 Порядок от ранжировщика/диверсификатора/блендера валидируется** (дубли, подмножество).

**3.22 `DenseFeatureMatrix.select(rows)`** — фильтрация после экстракции перенумеровывает матрицу вместе с кандидатами.

**3.23 `Extracted.matrix` — конкретный тип, не интерфейс.** Пайплайн владеет реализацией; `select` не попадает в контракт расширения.

**3.24 Стадий 16, а не 15.** §3 перечисляет 0..14 плюс 4b — 16 записей, но §4 и §17.1 говорят «15». Арифметика документа.

**3.25 Ядро: `resolve()`, движок: `build()`.** Ядро отказывается выдумывать ранкер; `engine/build()` заполняет пустые слоты.

**3.26 Пайплайн — прямолинейный код**, а не список объектов-стадий: последовательность фиксирована.

### Этап 4 и ревизия — новое

**3.27 RNG — `xoshiro128**`, а не `xoroshiro128+` из §23.4.** Тот 64-битный, а в JS нет 64-битных целых: нужен BigInt (аллокации, ~10× медленнее на пути, исполняемом для каждого кандидата) либо ручная арифметика по лимбам, которой пришлось бы доверять без референсных векторов. Та же семья, те же авторы, спроектирован под 32 бита. **Проверено аудитом:** реализация побайтово совпадает с референсом. `fork()` — чистая функция от seed'ов, а не от состояния родителя: иначе поток пользователя зависел бы от трафика.

**3.28 `softmax` не реализован.** §12 и план §22 его называют; в `NORMALIZERS` его нет. Единственный незакрытый пункт Этапа 4 — см. §5.

**3.29 Дефолтный ранкер сортирует всё, хотя куча в 75 раз быстрее.** Куча выигрывает только при K ≪ n, а *какой K допустим — не решение ранкера*: то, что он оставит, увидят диверсификация и exploration. Обрежь до страницы — MMR нечего подставлять; обрежь до топ-200 — discover не дотянется до хвоста. `topKRanker(pool)` есть для тех, кто пул знает. Вопрос принадлежит Этапу 7.

**3.30 `combiner.id` резолвится через `DEFAULT_COMBINERS`.** §10 объявляет ключ, но механизма резолва не описывает. Приоритет: слот (`setCombiner`) > `config.combiner.id` > дефолт. Опечатка в id отвергается на `build()` **и** на per-request override.

**3.31 `inspect()` помечает доменные стратегии.** §19 обещает, что использование `DomainScoringStrategy` «видно в `engine.inspect()`» — список голых id это обещание не выполнял.
...
2. **`normalizer` на стратегии.** Стратегия знает свою шкалу: `PopularityStrategy` вернёт 4 200 000 и должна попросить `rank` или `zscore`, а не полагаться на `minmax`, который расплющит колонку одним выбросом (§3.20 и тесты в `normalize.test.ts`).
3. **`softmax`** (§3.28) — дописать в `math/normalize.ts`, если стратегиям понадобится.
