...
> Состояние: **Этапы 0–6а закрыты. Следующий — Этап 7 (`@recoengine/diversity`).**
> Последняя ревизия: закрыт **Этап 6а** — `@recoengine/testing` (фикстуры + framework-agnostic contract-kit, 10 тестов); долг ручных фикстур погашен — golden-наборы Этапов 5–6 переведены на kit, добавлен прогон контрактов. ARCHITECTURE.md §21.1 сведён тем же заходом.
...
Ожидаемо: **617 тестов в 33 файлах**, `✓ Architecture check passed (7 packages)`, покрытие ядра **95.4% строк / 93.26% ветвей** (пороги в `vitest.config.ts`: 90/85/90/90). Из них 30 — Этап 5 (`strategies.test.ts`, +9 контрактных), 19 — Этап 6 (`modifiers.test.ts`, +3 контрактных), 10 — Этап 6а (`testing.test.ts`, dogfood kit).
...
Два пакета (`features`, `diversity`) — пустые каркасы с `export {}`. `strategies` (Этап 5), `modifiers` (Этап 6) и `testing` (Этап 6а) закрыты. Шестой, `recoengine`, — фасад `export * from '@recoengine/core'`; стратегии и модификаторы он пока не реэкспортирует (решить при Этапе 11, когда появится, что публиковать).
...
## 4. Следующий шаг — Этап 7 (`@recoengine/diversity`)

Критерий приёмки из §22: **MMR(λ=1) ≡ ranking**. Диверсификация (стадия 10) и блендинг (стадия 11): MMR, `AttributeQuotaDiversifier`, `SimilarityProvider`, `BucketBlender` (§14, §15 Discovery).

Что учесть, заходя:

1. **`SimilarityProvider` без слота в `Registry` (долг §5).** MMR нужна метрика близости; порт объявлен, слота нет. §19 даёт ему слот «many»; в §5 записано решение передавать его в опциях MMR. Здесь оно наконец исполняется — либо слот в реестре, либо опция диверсификатора; выбрать и внести в §14/§19 тем же заходом.
2. **MMR жадный, `O(k·n)`, применяется к top-N (N≈200), не ко всем.** `λ=1` → чистая релевантность (регресс-тест на эквивалентность ранжированию — это и есть критерий приёмки); `λ=0` → максимум разнообразия; дефолт 0.7. Длинный CPU-цикл ⇒ `throwIfAborted()` каждые ~1024 (§17.1, правило 3) — contract-kit Этапа 6а это проверит одной строкой.
3. **`cosine`/`jaccard`/`weightedJaccard` уже в `math/similarity.ts`.** `CosineSimilarity` по подпространству фичей — обёртка над ними; `JaccardSimilarity` держит множества сам (он доменный по определению, §14, и это законно).
4. **Дефолтного блендера нет (долг §5).** §18 включает exploration, исполнять некому — движок предупреждает `quota_unfilled`. `BucketBlender` раскладывает по бакетам (предикат из фичей), добор из соседнего + warning; `seed` из userId+даты (не `Math.random()`), стабильность в течение дня.
5. **Фикстуры и контракты — из `@recoengine/testing`.** Не катать движок руками (долг закрыт на 6а); диверсификатор/блендер прогнать через `assertHonoursCancellation` и добавить golden на MMR(λ=1)≡ranking.

**Хвосты, оставленные сознательно:** `softmax` не написан (§5, никому пока не понадобился); `recoengine`-фасад стратегии/модификаторы не реэкспортирует (решить на Этапе 11); `recovery()` из `decay.ts` не используется (восстановление выражено затуханием счётчика) — остаётся для линейной формы. `@recoengine/features` (Этап 8а) и доменные примеры (9–10) — после diversity и explainability.
...
| _(pending)_ | Этап 6а закрыт: `@recoengine/testing` (фикстуры + contract-kit), долг ручных фикстур погашен, ARCHITECTURE §21.1 сведён |
