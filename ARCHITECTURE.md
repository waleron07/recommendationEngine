...
Этапы 0–9 закрыты (все пакеты + первый доменный пример): 663 теста, покрытие ядра 95.4%, CI зелёный на Node 20/22/24, Bun и Deno. Текущее состояние работы, открытые долги и точка входа для продолжения — в [PROGRESS.md](./PROGRESS.md); этот документ описывает архитектуру, а не прогресс.
