...
> Описание системы, как она построена. Не AI и не LLM — детерминированная ранжирующая
> машина: даёшь пользователя, историю и набор кандидатов, получаешь упорядоченный список,
> где каждый объект умеет объяснить, почему он там. Документ описывает написанный код;
> где-то на него ссылаются номерами секций (`§17.2`, `§11.2`) прямо из исходников.
>
> Пользовательское руководство — [docs/РУКОВОДСТВО.md](./docs/РУКОВОДСТВО.md); публикация — [RELEASING.md](./RELEASING.md).
...
**Layout'а два, а не один.** Наивный «column-major везде» не работает: два потребителя ходят по матрице **перпендикулярно**: нормализатор идёт по одной фиче через все строки (нужен column-major), а косинус — по 128 измерениям одной строки (нужен row-major). При сплошном column-major эти 128 значений лежали бы на расстоянии `rows` друг от друга, и `vector()` мог бы вернуть только копию — аллокация на каждого кандидата, на функции, которую MMR зовёт O(страница × пул) раз.
...
/**
 * Модификатор получает ПИШУЩИЙ вид доски.
...
export interface Registry {
  ...
  /**
   * Фичи ПОЛЬЗОВАТЕЛЯ — отдельное пространство, а не раздел первого.
   *
  ...
  /** §10 требует WeightProvider в цепочке разрешения — слота не было. */
 ...
 *
 * шов между `resolve()` и `build()`. Ядро собирает и валидирует реестр —
...
export class EngineBuilder<P> implements Registry {
  ...
  /** Без него контейнер доступен только плагинам, и хост не подставит свой Clock. */
...
export interface EngineConfig {
  ...
  // описаны в §17.2, но в этот интерфейс не попадали.
...
### 11.1.1 А как же graph / session / sequence / PPR / ANN?
...
**Реализованы как фабрики-функции, а не классы.** Каждая стратегия — `affinityStrategy(...)`, `popularityStrategy(...)` и т.д., возвращающая `ScoringStrategy`. Это согласовано с функциональным экспортом ядра (`weightedSum`, `sortRanker`) и со структурной диспетчеризацией плагинов, которая узнаёт стратегию по методу `score`, а не по `instanceof` (§8.5, `plugin.ts` прямо предостерегает от `instanceof`). Конфигурируемые поля — `id` (он же ключ веса), фичи и `normalizer`; двум стратегиям одного вида нужны разные `id`, иначе они дерутся за один вес, и `addStrategy` это ловит.
...
### 11.3.1 Кто поставляет фичи (`@recoengine/features`)
...
**Уточнение kit.** `assertScoringStrategy` по умолчанию сам добавляет `payloadExtractor` для требуемых фич; когда фичи поставляет настоящий экстрактор/трансформ (как здесь), это коллизия объявлений. Добавлен флаг `featuresFromPlugins: true` — пропустить авто-payloadExtractor, фичи даёт `extraPlugins`.
...
### 14.1 Реализация (`@recoengine/diversity`)
...
### 15.1 Реализация (`@recoengine/modifiers`)

Три модификатора — `fatigueModifier`, `noveltyModifier`, `boostModifier`, — каждый фабрика, возвращающая `ScoreModifier` (как и стратегии). Три особенности реализации:
...
**Презентационная шкала округляется.** `toPresentation = Math.round(score × 100)` в одном месте (`stages/explanation.ts`), так что `score` и `baseScore` округляются одинаково и не спорят. Без округления получалось бы `96.55172413793103` — не число, которое читает человек, и не порог, с которым сравнивает правило `score > 80`.
...
|---|---|---|
| `CandidateProvider`, `required` (дефолт) | ошибка запроса | ошибка запроса |
...
| `ScoreNormalizer` | ошибка запроса | ошибка запроса |
...
const engine = createEngine<Track>()
  ...
  .configure({
    ...
    // не поставляет. Движок не молчит: пишет warning `quota_unfilled` — «сказано
...
### 21.1 Реализация (`@recoengine/testing`)

Пакет — две половины. **Фикстуры** (`catalogueOf`, `payloadExtractor`, `profileExtractor`, `constantStrategy`, `fixedClock`, `events`, `request`, `testEngine`, `rankedIds`, `scoreById`) — синтетические части движка, которые иначе каждый тест катал бы руками. **Контракты** — переиспользуемые проверки §20: `assertHonoursCancellation` (§17.1), `assertExtractorErrorPolicy` (§17.2, обе политики) — обязательные; `assertDeterministic`, `assertScoresWellFormed` — гигиена; `assertScoringStrategy`/`assertScoreModifier` — обёртки, прогоняющие порт через настоящий движок и применяющие всё перечисленное.
...
Каждая часть системы имеет одно машинно-проверяемое свойство, по которому видно, что она работает как задумано — не «код написан», а «делает то, что обещано»:

- **`domain/`** — плагин, требующий фичи, которую никто не производит, падает на `build()`, а не на запросе; `use()` после `build()` бросает.
- **`pipeline/`** — пустой движок отдаёт пустой результат с таймингами; прерванный сигнал даёт `AbortError` на каждой границе стадии.
- **`@recoengine/strategies`** — golden-тесты на синтетике: зафиксированный вход → зафиксированная выдача; cold start (нет истории) отбрасывает колонку и перевзвешивает остальные (§17.3).
- **`@recoengine/modifiers`** — кривые затухания и восстановления fatigue проверены на числах; модификатор «всё или ничего» (§17.2).
- **`@recoengine/diversity`** — **MMR(λ=1) ≡ чистое ранжирование**, доказано регресс-тестом: диверсификатор, ничего не меняющий, — тождественная функция.
- **`@recoengine/testing`** — порт проверяется на контракт одной строкой; обязательны прерванный сигнал (§17.1) и обе `errorPolicy` (§17.2).
- **Explainability** — объяснение сходится: **Σ contributions = baseScore** (golden-тест); `engine.explain(itemId)` отвечает, почему объекта нет в выдаче.
- **`@recoengine/features`** — доменно-нейтральные фичи (из истории/времени) переиспользуются music и ecommerce без изменений.
- **`examples/*`** — **приёмочный тест архитектуры**: доменный пример обслуживается написанием плагинов при **нуле изменений в `core`** (`git status packages/core/` пуст). Если для нового домена потребуется тронуть ядро — абстракция протекла. Два несвязанных домена (music, ecommerce) построены на одних и тех же стратегиях/модификаторах/диверсификаторах; отличаются только доменные экстракторы. Различие домены выражают данными, а не кодом ядра: e-commerce взвешивает типы событий (покупка ≫ корзина ≫ просмотр) внутри своего экстрактора, и стратегия об этом не знает.

---

## 23. Сквозные решения

Решения, действующие поперёк всех модулей. На них ссылается код (`§23.2`, `§23.4` и т.д.), потому что они — не деталь одного места, а контракт всей системы.

1. **Monorepo, не один пакет с subpath exports.** pnpm workspace, 8 пакетов — семь публикуемых (`core` без зависимостей, `strategies`, `features`, `modifiers`, `diversity`, `testing`, unscoped `recoengine`) плюс примеры. Ноль зависимостей у `core` иначе не удержать. Цена — сложнее релиз: changesets + правило зависимостей в CI.

2. **`score()` синхронна, навсегда.** Не оптимизация, а **охрана главного разделения** I/O → Extraction → Math. `async score()` — открытая дверь, в которую немедленно войдут «сходить в Redis за одним значением» и «дёрнуть сервис»; синхронная сигнатура делает нарушение невозможным, а не порицаемым. То же и у `FeatureTransform.apply`. Тот же приём, что с фильтрами (§6): инвариант охраняется типом.

3. **Лимиты — инвариант движка, без дефолтов.** `limits.maxCandidates`, `maxLimit`, `timeoutMs` обязательны; отсутствие → ошибка `build()`. Главное: бюджет **проталкивается** в провайдер (`RetrievalBudget`), а не применяется после (§6). Обрезка миллиона строк после `SELECT` защищает выдачу, но не базу — маскирует DOS, а не предотвращает. Оверрайды запроса валидируются тем же кодом, что `configure()`, — иначе потолок снимает тот, кого он ограничивает.

4. **Изоморфность.** `@recoengine/core` не импортирует Node API вообще, и это проверяется машиной, а не обещанием в README. Следствия — контракт:
   - RNG собственный (`xoshiro128**`), не `node:crypto`. Не `xoroshiro128+`: тот 64-битный, а в JavaScript нет 64-битных целых. Требование было к свойству (сеется, воспроизводим, быстр, не трогает окружение), а не к имени алгоритма.
   - `fork(seed)` — чистая функция от seed'ов, а не от состояния родителя: иначе поток пользователя зависел бы от трафика до него, и воспроизводимость исчезла бы.
   - Параллелизм внутри стадий — только `Promise.all`, без `worker_threads`. Реальный параллелизм — забота адаптера снаружи.
   - Всё I/O — за портами; адаптеры в отдельных пакетах.
   - `AbortSignal`/`AbortController` объявлены локальным `platform.d.ts` (`lib: ["ES2022"]`, `types: []`): это единственная платформенная поверхность ядра, и она заявлена явно, а не притащена через `lib.dom`.

5. **TS 5.9+, Node 20+, ESM-only, без CJS; изоморфность проверяется в CI.** lint-правило `noRestrictedImports` на `node:*` в `packages/core/**` — ошибка сборки. CI-матрица: **Node 20/22/24, Bun, Deno** (job `runtimes`, smoke) + **браузер** (job `browser`, headless Chromium — собранный ESM грузится `<script type="module">`, playwright ставится эфемерно в job). `package.json`: `"type": "module"`, `exports` без `require`-ветки, `"sideEffects": false`. Bun/Deno/браузер — строки в CI, а не поддержка «на словах»; только так это остаётся правдой.

6. **npm scope `@recoengine/*`.** Семь публикуемых пакетов; unscoped `recoengine` — meta-пакет «с батарейками» (реэкспорт `core` + все плагины), чтобы `npm i recoengine` работал из коробки и имя было защищено от squatting. Организация `recoengine` создана (owner `waleron`). 2FA `auth-and-writes` на аккаунте: публикация из CI по обычному токену не сработает — нужен granular token либо Trusted Publishing через OIDC (см. [RELEASING.md](./RELEASING.md)).

7. **Доменные примеры не публикуются.** Музыкальный и e-commerce домены живут в `examples/`, а не в `packages/`; `check-arch` запрещает publishable-пакетам зависеть от них. Публиковать доменный пакет — значит взять на себя поддержку домена и снова сделать библиотеку «музыкальной». Домен — это набор экстракторов и датасет; выделять его в самостоятельный пакет незачем.
