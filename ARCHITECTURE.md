...
| ✅ 9 | `examples/music` (домен схлопнут в пример) | Работает на реальном датасете; **0 изменений в core** |
| ✅ 10 | `examples/ecommerce` | **Доказано: 0 изменений в core** — `git status packages/core/` пуст |
| 🔄 11 | Docs, typedoc, README, бенчмарки, `v0.1.0` | Публикация в npm — **подготовлено, ждёт npm-auth** |

Этап 10 — не демо, а **приёмочный тест архитектуры**. Если для e-commerce потребуется тронуть `core`, значит абстракция протекла и её надо чинить до релиза. **Тест пройден (Этап 10 закрыт):** `examples/ecommerce` — второй, несвязанный домен (товары, бренды, категории, события `view`/`add_to_cart`/`purchase`) — обслужен теми же стратегиями/модификаторами/диверсификаторами, что и музыка; отличаются только доменные экстракторы и один prefilter «уже куплено». `git status packages/core/` пуст на коммите. Различие домены выражают не кодом ядра, а данными: e-commerce взвешивает типы событий (покупка ≫ корзина ≫ просмотр) внутри своего экстрактора — стратегия, читающая колонку, об этом не знает.

**Этап 9: `domain-music` схлопнут в `examples/music` (внесено при реализации).** План писал «`@recoengine/domain-music` + `examples/music`» — отдельный доменный пакет плюс приложение. На деле музыкальный домен — это пять экстракторов и датасет; выделять их в самостоятельный пакет значило бы завести публикуемую сущность ради того, что по определению не публикуется (§23.7: доменные пакеты — примеры, `check-arch` запрещает publishable-пакетам зависеть от `domain-*`). Всё живёт в `examples/music`: `catalogue.ts` (датасет), `extractors.ts` (единственный код, читающий `payload`), `engine.ts` (`buildMusicEngine` — сборка из готовых пакетов), `demo.ts`, `music.test.ts`. То же решение и по той же причине, что фабрики-функции вместо классов на Этапе 5: не заводить сущность, которой нечего делать. **Критерий выполнен буквально: `git status packages/core/` пуст — пример собрался и прошёл 7 e2e-тестов, не тронув ни строки ядра.** `examples/*` включены в `vitest` (тест, который не запускается, хуже отсутствующего, §21) и в корневой `tsconfig`; `check-arch` их не сканирует (они вне `packages/`), и это правильно — примеру можно тянуть любой `@recoengine/*`, но не наоборот.
...
Этапы 0–10 закрыты; Этап 11 подготовлен (версии `0.1.0`, CHANGELOG, фасад «с батарейками», 0 предупреждений) — остаётся сама публикация, ждущая npm-auth (см. [RELEASING.md](./RELEASING.md)). 672 теста, покрытие ядра 95.4%, CI зелёный на Node 20/22/24, Bun и Deno. Текущее состояние работы, открытые долги и точка входа для продолжения — в [PROGRESS.md](./PROGRESS.md); этот документ описывает архитектуру, а не прогресс.
