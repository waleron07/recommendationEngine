...
Этапы 0–10 закрыты (все пакеты + оба доменных примера): 670 тестов, покрытие ядра 95.4%, CI зелёный на Node 20/22/24, Bun и Deno. Осталось только Этап 11 — публикация. Текущее состояние работы, открытые долги и точка входа для продолжения — в [PROGRESS.md](./PROGRESS.md); этот документ описывает архитектуру, а не прогресс.
