...
│   │       │   ├── registry.ts          слоты расширения
│   │       │   ├── plugin.ts            Plugin, PluginHost, разрешение зависимостей
...
│   │       │   ├── decay.ts             exponential, gaussian, linear
│   │       │   ├── heap.ts              top-K
│   │       │   └── rng.ts               xoroshiro128+ (детерминированный)
│   │       └── engine/
│   │           ├── engine.ts            RecommendationEngine
│   │           ├── builder.ts           createEngine().use().configure().build()
│   │           └── defaults.ts          дефолтная сборка слотов
│   │
...
```ts
/**
 * Column-major Float64Array. rows = кандидаты, cols = колонки схемы.
 * column(key) отдаёт непрерывный срез — стратегия работает с ним как с вектором.
 */
...
Column-major, а не array-of-objects — принципиально: `ZScoreNormalizer` нормализует колонку одним проходом по непрерывной памяти. С массивом объектов это pointer chasing по 10k объектам на каждую из 30 фич.
...
export interface ScoreModifier {
  readonly id: string
  readonly kind: ContributionKind
  apply(board: ScoreBoard, set: CandidateSet, ctx: RequestContext): void
}
...
export class EngineBuilder<P> implements Registry {
  ...
  build(): RecommendationEngine<P>     // после вызова билдер запечатан
}

/** Что видит PIPELINE. Иммутабельный результат build(). */
...
```
use(plugin)            → в список, никаких сайд-эффектов
   ↓
build()                → 1. топологическая сортировка по dependsOn (цикл → ошибка)
                         2. merge configSchema, валидация конфига
                         3. register() каждого плагина в порядке сортировки
                            (плагин пишет через Registry в MutableFeatureSchema)
                         4. ВАЛИДАЦИЯ ГРАФА ФИЧЕЙ (см. ниже)
                         5. вычисление schema.version (FNV-1a)
                         6. ЗАМОРОЗКА: MutableFeatureSchema → FeatureSchema,
                                       EngineBuilder → ResolvedRegistry,
                                       билдер помечается sealed
   ↓
...
Повторный `build()` бросает `BuilderSealedError`. Повторный `use()` после `build()` — тоже: `EngineBuilder` после заморозки отвергает запись. Компилятор эту ошибку не поймает (ссылка на билдер могла быть сохранена), поэтому рантайм-проверка здесь обязательна как второй рубеж.
...
- **Детерминированный tie-break**: при равных score сортируем по `ItemId`. Иначе выдача «дёргается» между запросами (порядок из `Map` не гарантирован), и golden-тесты флакуют.
- Реализации: `TopKRanker` (дефолт), `ThresholdRanker` (`score > x`), `StratifiedRanker` (top-K внутри страт).
...
|---|---|---|
| `CandidateProvider`, часть | ошибка запроса | продолжаем на оставшихся + warning |
...
| `UserFeatureExtractor`, `optional` | ошибка запроса | то же + стратегии с `requiresProfile` отключаются |
...
| `ScoreModifier` | ошибка запроса | модификатор = 1.0 (нейтраль) + warning |
| `Diversifier` | ошибка запроса | пропускаем, отдаём чистое ранжирование + warning |
...
| 13 | `SimilarityProvider` | many | Метрика близости | да |
...
| 4 | `math/` — нормализаторы, similarity, mmr, rrf, decay, heap, rng | Property-тесты, бенчмарки |
...
4. ~~**Node-only или изоморфность?**~~ **РЕШЕНО: изоморфно.** `@recoengine/core` не импортирует Node API вообще (проверяется в CI). Следствия, зафиксированные как контракт:
   - RNG — собственный `xoroshiro128+`, не `node:crypto`.
